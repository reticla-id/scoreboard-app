...
import { matchFormatName, readMatchFormat } from "@/features/sports/formats";
...
export function SessionWorkspaceHeading({ session, active }: { session: Session; active: ActiveTab }) {
  ...
  return <header className="session-workspace-head">
    ...
    <div className="session-partner-mode"><span>MATCH TYPE</span><strong>{matchFormatName(matchFormat)}</strong>{matchFormat === "DOUBLES" && <><span>PARTNER MODE</span><strong>{partnerMode === "FIXED" ? "Fixed Partners" : "Random Partners"}</strong></>}</div>
