...
export function SessionWorkspaceHeading({ session, active }: { session: Session; active: ActiveTab }) {
  ...
  return <header className="session-workspace-head">
    <p className="eyebrow"><span className="dot" /> {session.sportConfig.name.toUpperCase()} / SESSION <span className={`status-chip status-${lifecycle.toLowerCase()}`}>{lifecycle}</span></p>
    <h1>{session.name}</h1>
