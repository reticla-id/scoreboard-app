...
export function SessionTabs({ sessionId, active }: { sessionId: string; active: "overview" | "players" | "matches" | "leaderboard" }) {
  return <nav className="session-tabs" aria-label="Session navigation">
    <Link href={`/sessions/${sessionId}`} aria-current={active === "overview" ? "page" : undefined}>Overview</Link>
    <Link href={`/sessions/${sessionId}/players`} aria-current={active === "players" ? "page" : undefined}>Players</Link>
    <Link href={`/sessions/${sessionId}/matches`} aria-current={active === "matches" ? "page" : undefined}>Matches</Link>
