...
function MatchLine({ match, sessionId, onGlossary, eventGlossary }: { match: MatchRow; sessionId: string; onGlossary: () => void; eventGlossary: EventGlossary }) {
  ...
  return <article className={`game-row ${status === "LIVE" ? "game-row-live" : ""} ${flash ? "game-row-flash" : ""}`}>
    ...
    {status === "LIVE" && <div className="score-entry" aria-label={`Score controls for match ${match.position}`}>
      ...
      <div className="score-actions">{(score.a > 0 || score.b > 0 || hasTrackedEvents) && <button className="text-link" type="button" onClick={() => queue({ kind: "reset" })}>{hasTrackedEvents ? "Reset score & points" : "Reset score"}</button>}<button className="button button-small" type="button" disabled={pendingCount > 0 || busy} onClick={() => advance("FINISHED")}>Finish match ↗</button></div>
    </div>}
...
export function MatchWorkspace({ sessionId, rounds, current, page, pages, minimumPlayers, eventGlossary, completed = false }: { sessionId: string; rounds: RoundSummary[]; current: RoundView | null; page: number; pages: number; minimumPlayers: number; eventGlossary: EventGlossary; completed?: boolean }) {
  ...
  if (!current) return <section className="games-empty"><span className="panel-index">NO GAMES YET</span><h2>{completed ? "NO MATCHES RECORDED." : <>PLAYERS FIRST.<br />GAMES NEXT.</>}</h2><p className="muted">{completed ? "This session is finished. Its schedule is locked." : `Save at least ${minimumPlayers} players, then generate a doubles round from Players.`}</p><Link className="button" href={`/sessions/${sessionId}/players`}>View players ↗</Link></section>;
  const total = rounds.reduce((count, round) => count + round.matchCount, 0);
  const base = `/sessions/${sessionId}/matches?round=${current.number}`;
  return <div className="games-workspace"><div className="games-heading"><div><span className="panel-index">COURT VIEW</span><h2>MATCHES.</h2><p className="muted">{rounds.length} round{rounds.length === 1 ? "" : "s"} · {total.toLocaleString("en-US")} games</p></div>{!completed && <Link className="text-link" href={`/sessions/${sessionId}/players`}>Players / generate another round →</Link>}</div>
    ...
    <section className="round-section" id={`round-${current.number}`} aria-labelledby="current-round-heading"><div className="round-heading"><div><span className="panel-index">ROUND {String(current.number).padStart(2, "0")}</span><h3 id="current-round-heading">ROUND {current.number}</h3></div><p>{current.matchCount.toLocaleString("en-US")} MATCH{current.matchCount === 1 ? "" : "ES"}</p></div>
