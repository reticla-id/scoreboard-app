...
export async function SessionBrowsePage({ view, query }: { view: SessionView; query: { page?: string; date?: string; court?: string } }) {
  ...
  return <main className="site-shell workspace-page">
    ...
    <BackLink href="/home" />
    <header className="browse-page-head"><div><p className="eyebrow"><span className="dot" /> {history ? "PAST SESSIONS" : "YOUR COURT CALENDAR"}</p><h1>{history ? "HISTORY." : "SESSIONS."}</h1><p className="muted">{history ? "Past and finished sessions, newest first. Historical sessions stay editable until you finish them." : "Today and upcoming sessions, sorted by date and time."}</p></div>{!history && <Link className="button" href="/sessions/new">Host Session <ArrowUpRightIcon /></Link>}</header>
    <div className="browse-toolbar"><span>{total} {history ? "historical" : "current"} session{total === 1 ? "" : "s"}</span><SessionFilters key={`${filters.date ?? ""}|${filters.court ?? ""}`} basePath={path} initialDate={filters.date} initialCourt={filters.court} /></div>
    <SessionRows sessions={sessions} view={view} grouped emptyTitle={history ? "NO HISTORY YET" : "NO SESSIONS FOUND"} emptyText={filters.date || filters.court ? "Try another date or court, or reset your filters." : history ? "Past and finished sessions will appear here." : "Host a session to get started."} />
