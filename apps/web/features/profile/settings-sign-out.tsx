...
function SignOutButton() {
  ...
  return <button className="button button-danger button-small" type="submit" disabled={pending}>{pending ? "Signing out…" : "Sign Out"}</button>;
}

export function SettingsSignOut() {
  const dialog = useRef<HTMLDialogElement>(null);
  return <footer className="settings-sign-out">
    <div className="settings-sign-out-inner"><button className="settings-sign-out-button" type="button" onClick={() => dialog.current?.showModal()}>Sign Out</button></div>
