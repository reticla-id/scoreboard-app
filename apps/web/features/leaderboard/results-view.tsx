...
export function ResultsView({ sessionId, standings, playerRanking, partnerMode, scoredMatches, trackedEvents, eventGlossary }: {
  ...
  return <section className="standings results-view" aria-labelledby="results-heading">
    ...
    {view === "leaderboard" && (scoredMatches === 0 ? <div className="standings-empty"><h3>NO RESULTS YET.</h3><p className="muted">Finish a scored match to start the leaderboard.</p><Link className="button" href={`/sessions/${sessionId}/matches`}>Go to matches ↗</Link></div> : <>
    ...
    {view === "stats" && (trackedEvents === 0 ? <div className="standings-empty"><h3>NO POINTS TRACKED.</h3><p className="muted">Open Advanced Tracking on a live match, then finish it to see player outcomes here.</p><Link className="button" href={`/sessions/${sessionId}/matches`}>Go to matches ↗</Link></div> : <>
