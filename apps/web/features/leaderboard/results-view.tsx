...
export function ResultsView({ sessionId, sport, standings, playerRanking, partnerMode, scoredMatches, trackedEvents, eventGlossary, supportsPlayerStats = true, publicView = false }: {
  sessionId: string;
  sport: string;
  ...
  return <section className="standings results-view" aria-labelledby="results-heading">
    ...
    {view === "leaderboard" && (scoredMatches === 0 ? <>{!publicView && <div className="results-actions">{refresh}</div>}<div className="standings-empty"><h3>NO RESULTS YET.</h3><p className="muted">Finish a scored match to start the leaderboard.</p>{!publicView && <Link className="button" href={`/sessions/${sessionId}/matches`}>Go to matches <ArrowUpRightIcon /></Link>}</div></> : <>
      <div className="results-actions">{!publicView && refresh}<TableExportActions type="leaderboard" rows={standings} partnerMode={partnerMode} sport={sport} /></div>
      <table className={`results-table match-standings-table${padel ? " padel-standings-table" : ""}`}><caption className="sr-only">{fixed ? "Fixed partner" : "Player"} standings ranked by win rate</caption><colgroup><col className="rank-col" /><col className="player-col" /><col className="wins-col" /><col className="losses-col" />{padel && <col className="total-col" />}<col className="diff-col" /><col className="rate-col" /></colgroup><thead><tr><th scope="col">Rank</th><th scope="col">{fixed ? "Partners" : "Player"}</th><th scope="col"><abbr title="Wins">W</abbr></th><th scope="col"><abbr title="Losses">L</abbr></th>{padel && <th scope="col">Total Score</th>}<th scope="col">Diff</th><th scope="col">Win %</th></tr></thead><tbody>{standings.map((row, index) => <tr key={row.id} className={index === 0 ? "leader-row" : undefined}><td className="results-rank"><Rank index={index} /></td><th scope="row" className="results-name">{row.name}</th><td>{row.wins}</td><td>{row.losses}</td>{padel && <td>{row.gamesWon}</td>}<td>{signed(row.difference)}</td><td className="results-rate">{row.winPercent.toFixed(1)}%</td></tr>)}</tbody></table>
      <ol className="rankings-mobile" aria-label={fixed ? "Fixed partner standings" : "Player standings"}>{standings.map((row, index) => <li key={row.id} className={index === 0 ? "leader-row" : undefined}><span className="results-rank"><Rank index={index} /></span><strong className="results-name">{row.name}</strong><strong className="results-net">{row.winPercent.toFixed(1)}%</strong><span className="results-secondary">{row.wins} W · {row.losses} L</span><span className="results-events"><small>{padel ? "TOTAL SCORE / DIFF" : "DIFF / PLAYED"}</small><b>{padel ? `${row.gamesWon} / ${signed(row.difference)}` : `${signed(row.difference)} / ${row.matchesPlayed}`}</b></span></li>)}</ol>
