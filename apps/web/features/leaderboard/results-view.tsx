...
export function ResultsView({ sessionId, standings, playerRanking, partnerMode, scoredMatches, trackedEvents, eventGlossary }: {
  ...
  return <section className="standings results-view" aria-labelledby="results-heading">
    <div className="results-segment" role="group" aria-label="Session results view">
    ...
    {view === "stats" && (trackedEvents === 0 ? <div className="standings-empty"><h3>NO POINTS TRACKED.</h3><p className="muted">Open Advanced Tracking on a live match, then finish it to see player outcomes here.</p><Link className="button" href={`/sessions/${sessionId}/matches`}>Go to matches <ArrowUpRightIcon /></Link></div> : <>
      <TableExportActions type="stats" rows={playerRanking} />
    ...
    {view === "stats" && <details className="ranking-glossary"><summary>Ranking &amp; stats key <span aria-hidden="true">i</span></summary><dl><div><dt>Net Score</dt><dd>Winners − Unforced Errors − Double Faults. Forced Errors do not change this score.</dd></div><div><dt>Efficiency</dt><dd>Winners ÷ (Winners + Unforced Errors + Double Faults). A dash means no qualifying outcomes yet.</dd></div>{eventGlossary.map((item) => <div key={item.code}><dt>{item.code} · {item.name}</dt><dd>{item.description}</dd></div>)}</dl></details>}
  </section>;
}
