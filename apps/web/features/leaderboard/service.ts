...
import { calculateTennisScore, readTennisPointHistory, tennisGameTotals } from "@/features/sports/tennis/scoring";

type OwnedSession = Awaited<ReturnType<typeof getOwnedSession>>;
type LeaderboardSession = Pick<OwnedSession, "id" | "sportConfig" | "partnerMode" | "fixedPairs" | "matchFormat">;
...
export async function calculateSessionLeaderboard(session: LeaderboardSession) {
  ...
  const [players, finishedMatches, eventCounts] = await Promise.all([
    ...
    db().match.findMany({ where: { sessionId, status: "FINISHED" }, select: {
      status: true, scoreA: true, scoreB: true, scoreState: true,
  ...
  const matches: ResultMatch[] = finishedMatches.map((match) => {
    const tennisScore = session.sportConfig.rules.code === "TENNIS"
      ? calculateTennisScore(readTennisPointHistory(match.scoreState), session.sportConfig.rules.scoring)
      : null;
    const games = tennisScore ? tennisGameTotals(tennisScore) : null;
    return {
      status: match.status,
      scoreA: games?.gamesA ?? match.scoreA,
      scoreB: games?.gamesB ?? match.scoreB,
      winner: tennisScore?.winner ?? null,
      teamA: [match.teamA.playerOneId, match.teamA.playerTwoId].filter((id): id is string => id !== null),
      teamB: [match.teamB.playerOneId, match.teamB.playerTwoId].filter((id): id is string => id !== null),
    };
  });
  ...
  const partnerMode = session.matchFormat === "SINGLES" ? "RANDOM" : readPartnerMode(session.partnerMode);
  return {
    ...
    partnerMode,
    scoredMatches: matches.filter((match) => match.scoreA !== null && match.scoreB !== null && (match.winner || match.scoreA !== match.scoreB)).length,
    trackedEvents: counts.reduce((total, entry) => total + entry.count, 0),
    supportsPlayerStats: session.sportConfig.rules.code === "PADEL",
  };
}
