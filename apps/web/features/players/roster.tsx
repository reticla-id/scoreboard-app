...
function SaveButton({ label }: { label: string }) {
  ...
  return <button className="button button-small" type="submit" disabled={pending}>{pending ? "Saving…" : label}</button>;
}
...
export function Roster({ players, sessionId, minimumPlayers, sportName, readOnly = false, lockReason }: { players: RosterPlayer[]; sessionId: string; minimumPlayers: number; sportName: string; readOnly?: boolean; lockReason?: string }) {
  ...
  return <section className="roster-workspace" aria-labelledby="roster-heading">
    {players.length > 0 && <div className="roster-topline"><div><p className="panel-index">{readOnly ? "ROSTER / READ ONLY" : "SAVED ROSTER"}</p><h2 id="roster-heading">{players.length} PLAYER{players.length === 1 ? "" : "S"}</h2>{lockReason && <p className="muted roster-lock-note">{lockReason}</p>}</div>{!readOnly && mode === "closed" && <div className="roster-entry-actions"><button className="button button-secondary button-small" type="button" onClick={() => setMode("manual")}>+ Add players</button><button className="text-link" type="button" onClick={() => setMode("import")}>Import list</button></div>}</div>}
    {players.length === 0 && <div className="roster-simple-empty"><h2 id="roster-heading">{readOnly ? "NO PLAYERS RECORDED." : "WHO’S PLAYING TODAY?"}</h2><p className="muted">{readOnly ? lockReason ?? "This session is finished, so its roster is locked." : `${minimumPlayers} players make the first ${sportName.toLowerCase()} doubles match.`}</p>{!readOnly && mode === "closed" && <div className="roster-empty-actions"><button className="button" type="button" onClick={() => setMode("manual")}>Add players ↗</button><button className="button button-secondary" type="button" onClick={() => setMode("import")}>Import list</button></div>}</div>}
    {!readOnly && mode === "manual" && <div className="entry-reveal"><div className="entry-reveal-head"><h3>ADD PLAYERS</h3><button className="text-link" type="button" onClick={() => setMode("closed")}>Done</button></div><form action={action} ref={formRef} className="quick-add-form"><div className="session-form-field"><label htmlFor="player-name">Player name</label><input ref={inputRef} id="player-name" name="name" type="text" maxLength={80} required autoFocus placeholder="Type a name" autoComplete="off" /></div><SaveButton label="Add ↗" /></form><p className="field-note">Press Enter to add the next player.</p>{state.error && <p className="message error" role="alert">{state.error}</p>}{state.success && <p className="message success" role="status">{state.success}</p>}</div>}
    {!readOnly && mode === "import" && <div className="entry-reveal"><div className="entry-reveal-head"><h3>IMPORT LIST</h3><button className="text-link" type="button" onClick={() => setMode("closed")}>Done</button></div><ImportPanel sessionId={sessionId} players={players} /></div>}
