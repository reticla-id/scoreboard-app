...
export default async function SessionDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="site-shell workspace-page">
    ...
    <section className="session-court-progress" aria-label="Session progress"><div className="session-progress-intro"><span className="panel-index">{phase.toUpperCase()} SESSION</span><h2>{session.completedAt ? "SESSION FINISHED." : liveCount ? "GAMES IN PLAY." : roundCount ? "MATCHES ARE READY." : playerCount < minimumPlayers ? "START WITH PLAYERS." : needsPartners ? "SET PARTNERS." : "READY FOR ROUND 1."}</h2><p className="muted">{session.completedAt ? "Roster and rounds are locked. You can still correct scores and review results." : roundCount ? `${roundCount} round${roundCount === 1 ? "" : "s"} generated. Open Matches to run games and record scores.` : playerCount < minimumPlayers ? `Build your roster. ${minimumPlayers} players are needed for ${session.sportConfig.name.toLowerCase()} doubles.` : needsPartners ? "Pair every player on the Players screen before generating a round." : "The roster is ready. Generate the first round from Players."}</p><Link className={session.completedAt ? "button" : "button button-secondary"} href={roundCount ? `/sessions/${session.id}/matches` : `/sessions/${session.id}/players`}>{roundCount ? "Open matches" : session.completedAt ? "View players" : playerCount < minimumPlayers ? "Add players" : needsPartners ? "Set partners" : "Review roster"} ↗</Link></div><div className="session-metrics" aria-label="Session totals"><div><span>PLAYERS</span><strong>{playerCount}</strong></div><div><span>TOTAL MATCHES</span><strong>{totalMatches}</strong></div><div><span>UPCOMING</span><strong>{upcomingCount}</strong></div><div><span>LIVE</span><strong>{liveCount}</strong></div><div><span>FINISHED</span><strong>{finishedCount}</strong></div></div></section>
