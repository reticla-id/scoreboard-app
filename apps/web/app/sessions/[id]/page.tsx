...
import { LiveSessionControl } from "@/features/sharing/live-session-control";
import { getActiveSessionShare } from "@/features/sharing/data";
...
export default async function SessionDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [roster, roundCount, matchCounts, share] = await Promise.all([
    partnerMode === "FIXED" && matchFormat === "DOUBLES"
    ...
    getActiveSessionShare(session.id),
  ]);
  ...
  const needsPartners = matchFormat === "DOUBLES" && partnerMode === "FIXED" && Array.isArray(roster) && !!validateFixedPairs(roster, fixedPairs, minimumPlayers).error;
